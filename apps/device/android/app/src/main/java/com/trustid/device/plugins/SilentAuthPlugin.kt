...
/**
 * Hardware-backed silent auth: biometric unlock ? Keystore ES256 sign of server challenge.
...
class SilentAuthPlugin : Plugin() {
      ...
      object : BiometricPrompt.AuthenticationCallback() {
        override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult) {
          try {
            ...
            val ret = JSObject()
            ret.put("keyId", KEY_ID)
            ret.put("signature", Base64.encodeToString(der, Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING))
            call.resolve(ret)
          } catch (e: Exception) {
        ...
        override fun onAuthenticationFailed() {
          // Keep prompt open; do not reject yet.
        }
      },
    )
  ...
  private fun exportPublicKeySpki(): String {
    ...
    // Certificate public key encoding is already SubjectPublicKeyInfo (SPKI).
    val spki = pub.encoded
    return Base64.encodeToString(spki, Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING)
  }
}

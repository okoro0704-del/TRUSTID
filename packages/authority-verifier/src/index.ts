/**
 * Shared Digi authority token verifier for independent consumers (ElfCom, TV, ù).
 * TrustID keys are NEVER used here ù only Digi authority JWKS.
 */

import { createRemoteJWKSet, importJWK, jwtVerify, type JWK, type JWTVerifyGetKey } from "jose";

/** Logical issuer ù not TrustID. Keep stable for T3/T4 consumers. */
...
export type VerifyAuthorityInput = {
  ...
  resource: string;
  /** Expected actor key (type:id). Required ù do not trust X-Actor alone. */
  actor: string;
...
export async function verifyAuthority(
  ...
  try {
    ...
    // Identity assertions must never pass ù require authority-specific claims
    ...
    // Exact resource match ù no prefix / substring

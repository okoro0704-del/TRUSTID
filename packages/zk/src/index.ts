...
function attestKey(issuerSecret: string): Buffer {
  return createHash("sha256").update(`trustid-zk-v1:${issuerSecret}`).digest();
}

function signSignals(issuerSecret: string, publicSignals: string[]): string {
...
export function proveTrustTierGte(input: {
  ...
  // Shape compatible with snarkjs Groth16 proof JSON for RP parsers
  return {
    protocol: PROTOCOL,
    circuit: CIRCUIT_ID,
    publicSignals,
